...
import coding
import struct
from entropy import symbol_occurrences
import Code
...
def write_occurrences(occurrences, filename):
    ...
    for i in occurrences:
        ...
        stream.write(struct.pack('i',occurrences[i]))
    stream.close()
...
def read_occurrences(filename):
    ...
    with open(filename,'rb') as file:
        text=file.read(5)
        while len(text)>0:
            tot=struct.unpack('i',text[1:])
            res[bytes([text[0]])]=tot[0]
            text=file.read(5)
    return res
...
def huffman_encode(filename, out_filename):
    ...
    with open(filename, 'rb') as stream:
        occu=symbol_occurrences(stream)
        tree=create_huffman_tree(occu)

    # Création du codage de Huffman
        C=huffman_coding(tree)
    # Ecriture du fichier stockant le dictionnaire d'occurrences
        write_occurrences(occu,out_filename+'.code')
    # Mise de la tête de lecture au début du fichier pour le reparcourir
    with open(filename,'rb') as stream1:
        with open(out_filename,'wb') as stream2:
    # Parcours du fichier et encodage des symboles lus
            lettre=stream1.read(1)
            while lettre != b'': # While we're not at the end of the file
                temp += C.code(lettre)
                if len(temp) >= 8:
                    temp=Code.flush_binary_string(temp, stream2)
                lettre=stream1.read(1)
...
def prefix_tree_decoding(bits, tree):
    ...
    for bit in bits:
        lettre += bit
        try:
            syllabe = C.decode(lettre)
            mot += syllabe ; lettre = ""
        except coding.Undecodable_word :
            pass
    return mot
...
if __name__ == '__main__':
    ...
    huffman_encode("codage.bmp", "cigale_code.bmp")
    huffman_decode("cigale_code.bmp", "cigale_decode.bmp")

# Q13
# Le fichier huffman_skeleton une fois encodé ne fait plus que 4,534 bytes
# Le fichier original faisait 7683 bytes
# Cela veut dire que l'on a eu une reduction de 41% et donc une longueur
# moyenne de 8*4534/7683 = 4.7208 bits/byte
# L'entropie du fichier d'origine est de 4.684358 bits/byte
# Celle du fichier compréssé est de 7,792766 bits/byte
# Si il était encodé avec un codage dont la longueur moyenne serait exactement l'entropie du fichier d'origine, sa longueur serait de 4498.74031425 bytes
# Si on utilisait un codage de longueur fixe minimale, le fichier compressé
# ferait:

# Q14
# Les fichiers sont bien identiques

"""
Si on code entropy_skeleton.py, on se retrouve avec un fichier de 1362 bytes (originalement 2243 bytes)
On a donc une longueur moyenne de 4,857776759 bits par byte
Ce qui nous fait une différence de 0.028440785 bitS/byte avec le résultat théorique
"""
