...
def code_word(word, coding):
    ...
    #     return ""
    # else:
    #     return coding.code(word[0]) + code_word(word[1:]) 
...
#Q13
print("Q13", decode_fixed_length("0101100000111110111100111010000101100000011011001100111100010111001111010000010011111000110010011111010111111101110101001010110001010000010010001111110001000111000001000101111001000110110011010000010010001", coding1), "\n")
...
def write_bits(stream, bits):
    """
    Write bits (a number multiple of 8) in a writable binary stream.
    ...
    >>> import tempfile; r=tempfile.NamedTemporaryFile(mode="rb+")
    ...
    [223, 223, 1]
    """
    assert "b" in stream.mode and stream.writable(), "The stream is not opened correctly {}".format(stream.mode)
    if len(bits) < 8:
        return bits
    else:
        stream.write(bytes(binary_to_bytes(bits[0:8])))
...
def read_bits(stream):
    ...
    ''
    """
    text = stream.read(1) # Reads the first byte
    try:
        return byte_to_binary(text[0])
    except:
        return ''
...
def uncomplete_byte(bits):
    ...
    Traceback (most recent call last):
    ...
    AssertionError: One can only uncomplete a byte
    """
    assert len(bits) == 8 and "1" in bits, "One can only uncomplete a byte"
...
def remove_completion(bits):
    ...
    """
    l = len(bits)
    assert l >= 8, "remove_completion: There is no bytes in bits!"
