...
#Q8
# Le fait d'avoir deux listes nous permet de mieux catégoriser
# les arbres avec des noeuds et des arbres avec un simple symbole
# De plus, lorsqu'on rajoute un arbre à `tree_list`, nous avons
# la garantie que c'est l'élément avec le plus d'occurence de la liste
# En effet, arbres précédents étaient fait avec moins d'occurence.
# Ainsi, la liste `tree_list` est triée sans que l'on ai à faire quoi que ce soit
# Tandis que si on utilisait une seule liste, on devrait la retrier (ou au moins insérer le nouvel arbre dans l'ordre) à chaque fois
# Pour conclure, le fait d'avoir deux listes nous permet d'économiser
# du temps de calcul sur les tris.
...
def write_occurrences(occurrences, filename):
    ...
    with open(filename, 'wb') as stream:
        for occurence in occurrences:
            assert occurence[0] < 256
            stream.write(bytes([ord(occurence[0])]))
            stream.write(struct.pack('i', occurence[1]))
...
def read_occurrences(filename):
    ...
    with open(filename, 'rb') as stream:
        text = []
        text.append(stream.read(1), struct.unpack('i', stream.read(4)))
